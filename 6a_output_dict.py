...
runs = ['run_A', 'run_B', 'run_C', 'run_D', 'run_E', 'run_fiducial', 'run_original']
...
for run in runs:
    ...
    ## CCSN run
    if run == 'run_original':
        for file in os.listdir(data_dir + run):
            if file.startswith('DD'):
                datasets.append(data_dir + run + '/DD' + file.split('DD')[1] + '/output_' + file.split('DD')[1])
    ## NSM runs
    else:
        for file in os.listdir(data_dir + run):
            if file.startswith('DD'):
...
fns = yt.DatasetSeries(datasets)#, parallel=2)
...
if yt.is_root():
    ...
    for i, run_name in enumerate(runs):
        ...
        if run_name != 'run_original':
            #run_name = j.split('/')[-1]
            param_info[run_name] = {}
            param_file = data_dir + run_name + "/PopIII_params.py" 
            f = open(param_file, "r")
